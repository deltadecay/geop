...
// An Imagick implementation of ImageFactory and Canvas.
...
class ImagickFactory implements ImageFactory
{
	...
	public function cropImage($image, $width, $height, $offsetx, $offsety)
	{
		if($image != null)
		{
			// Clone so that original image is not modified
			$cropped = clone $image;
			$cropped->cropImage($width, $height, $offsetx, $offsety);
			return $cropped;
		}
		return null;
	}
	...
	public function resizeImage($image, $width, $height)
	{
		if($image != null)
		{
			...
			// Clone so that original image is not modified
			$resized = clone $image;
			$resized->resizeImage(intval($width), intval($height), \Imagick::FILTER_LANCZOS, $blur);
			return $resized;
		}
		return null;
	}
